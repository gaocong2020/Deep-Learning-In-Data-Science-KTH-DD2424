...
% temp = [0,0.125,0.25,0.375,0.5,0.625,0.75,0.875];
lambda = 10^-4:2*10^-4:10^-2;
validation_acc = zeros(1,length(lambda));
for k = 1:length(lambda)
%     lambda = 5e-3;
%     le = le_min + (le_max - le_min)*temp(k);
%     lambda = 10^le
    %Parameters
    eta_min = 1e-5;
    eta_max = 0.1;
...
%     J_validation = zeros(1, n_epochs);
    ...
    %     [training_X, training_Y, training_y] = shuffle(training_X, training_Y, training_y);
        for j=1:size(training_X,2)/n_batch
            ...
            a = MiniBatchGD(Xbatch, Ybatch, a, lambda(k), eta);
            t = t + 1;
        end
...
%         J_validation(i) = ComputeCost(validation_X, validation_Y, a, lambda);
    ...
    validation_acc(k) = ComputeAccuracy(validation_X, validation_y, a);
...
lambda
validation_acc
plot(1:length(lambda), validation_acc);

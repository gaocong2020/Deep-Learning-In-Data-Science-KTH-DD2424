...
for i=1:n_epochs
    ...
    for j=1:size(training_X,2)/n_batch
        ...
        Xbatch = Xbatch + 0.25*randn(size(Xbatch));
...
dropout = (randn(size(s_BN{1}))<p)/p;
s_BN{1} = dropout.*s_BN{1};
x{1} = s_BN{1};
x{1}(find(s_BN{1}<0)) = 0;
for i = 2:k-1
...
%     dropout = (randn(size(s_BN{i}))<p)/p;
%     s_BN{i} = dropout.*s_BN{i};
    x{i} = s_BN{i};
    x{i}(find(s_BN{i}<0)) = 0;
end
...
for i = 1:k
%     W{i} = 0.01*randn(n_hiddens(i+1), n_hiddens(i));

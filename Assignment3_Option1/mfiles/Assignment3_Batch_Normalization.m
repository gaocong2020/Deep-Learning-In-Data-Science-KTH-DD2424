...
% Initialization
[W, b, gamma, beta] = initialization(d, n_hiddens, k);
...
ns = 900;
alpha = 0.9;
[P, x, mu_av, var_av, s, s_BN] = EvaluateClassifier(training_X(:, 1:n_batch), W, b, gamma, beta, k);
for i=1:n_epochs
    ...
    for j=1:size(training_X,2)/n_batch
        ...
        [W, b, gamma, beta, mu, variance] = MiniBatchGD(Xbatch, Ybatch, W, b, gamma, beta, lambda, eta, k);
        for p = 1:k-1
...
%         end
    ...
    J_train(i) = ComputeCost(training_X, training_Y, W, b, gamma, beta, lambda, k);
    J_validation(i) = ComputeCost(validation_X, validation_Y, W, b, gamma, beta, lambda, k);
...
acc_training = ComputeAccuracy(training_X, training_y, W, b, gamma, beta, k)
acc_validation = ComputeAccuracy(validation_X, validation_y, W, b, gamma, beta, k)
acc_test = ComputeAccuracy(test_X, test_y, W, b, gamma, beta, k)
...
function [P, x, mu, variance, s, s_BN] = EvaluateClassifier(X, W, b, gamma, beta, k)
n = size(X, 2);
s = cell(k, 1);
x = cell(k-1, 1);
mu = cell(k-1, 1);
...
s_extra{1} = (gamma{1}*ones(1, n)).*s_BN{1} + beta{1}*ones(1, n);
x{1} = s_extra{1};
x{1}(find(s_extra{1}<0)) = 0;
for i = 2:k-1
    s{i} = W{i}*x{i-1} + b{i}*ones(1, n);
    mu{i} = mean(s{i}, 2);
    variance{i} = var(s{i}, 0, 2)*(n - 1)/n;
    s_BN{i} = diag((variance{i} + eps).^(-0.5))*(s{i} - mu{i}*ones(1, n));
    s_extra{i} = (gamma{i}*ones(1, n)).*s_BN{i} + beta{i}*ones(1, n);
    x{i} = s_extra{i};
    x{i}(find(s_extra{i}<0)) = 0;
end
...
[P, ~, ~, ~, ~, ~] = EvaluateClassifier(X, W, b, gamma, beta, k);
...
%Compute gradients
function [grad_W, grad_b, grad_gamma, grad_beta, mu, variance] = ComputeGradients(X, Y, W, b, gamma, beta, k, lambda)
n = size(X, 2);
grad_W = cell(k, 1);
grad_b = cell(k, 1);
grad_gamma = cell(k-1, 1);
grad_beta = cell(k-1, 1);
[P, x, mu, variance, s, s_BN] = EvaluateClassifier(X, W, b, gamma, beta, k);
G = P - Y;
...
for i = k-1:-1:1
    % BatchNormBackPass
    grad_gamma{i} = (G.*s_BN{i})*ones(n,1)/n;
    grad_beta{i} = G*ones(n,1)/n;
    G = G.*(gamma{i}*ones(1, n));
...
% Mini-batch gradient descent algorithm
function [update_W, update_b, update_gamma, update_beta, mu, variance] = MiniBatchGD(X, Y, W, b, gamma, beta, lambda, eta, k)
[grad_W, grad_b, grad_gamma, grad_beta, mu, variance] = ComputeGradients(X, Y, W, b, gamma, beta, k, lambda);
update_W = W;
update_b = b;
update_gamma = gamma;
update_beta = beta;
...
for i = 1:k-11
    update_gamma{i} = W{i} - eta*grad_gamma{i};
    update_beta{i} = b{i} - eta*grad_beta{i};
end
end
...
%Initialization
function [W, b, gamma, beta] = initialization(d, n_hiddens, k)
W = cell(k, 1);
b = cell(k, 1);
gamma = cell(k-1, 1);
beta = cell(k-1, 1);
...
for i = 1:k
    ...
    b{i} = (1/sqrt(n_hiddens(i)))*randn(n_hiddens(i+1), 1);
end
for i = 1:k-1
    gamma{i} = (1/sqrt(n_hiddens(i+1)))*randn(n_hiddens(i+1), 1);
    beta{i} = (1/sqrt(n_hiddens(i+1)))*randn(n_hiddens(i+1), 1);
end
end
